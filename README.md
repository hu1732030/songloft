...
> 仓库：`hu1732030/songloft` · 分支：`main`  
> 说明：以下为自托管定制增强，**不改动第三方 API 契约**。

### 安全与账号
- **游客安全**：IP / client_id 黑名单 + 游客登录审计（成功 / 失败 / 验证码 / 限流 / 命中黑名单）
- **用户登录审计**：正式账号登录成功 / 失败写入同一审计列表（含 IP、用户名、时间；不记密码）
- **登录页**：协议默认勾选，登录与「游客试听」可直接点击

### 曲库与播放
- **删歌不再删磁盘文件**：曲库删除 / 批量删除 / 去重 / 删歌单连带清孤儿曲，仅删库记录，保留本地音频
- **歌曲分享**：曲库 / 播放器可复制分享链接（`?song=<id>`）；对方打开后登录或游客试听，自动定位并播放该曲（链接基于当前访问域名生成）

### Web 体验
- **网页快捷键**：浏览器前台支持空格播放/暂停、Ctrl+←/→ 上一曲/下一曲等（设置中可改；后台标签页请用系统媒体键）
